...
final class HTTPMessageSourceTests: XCTestCase {
    ...
    func testLocaleIsResolvedOnEveryFetch() {
        let transport = RecordingTransport(outcomes: [.success(Data("{}".utf8)),
                                                      .success(Data("{}".utf8))])
        var language = "en"
        let source = HTTPMessageSource(transport: transport,
                                       appVersion: "1.2.3",
                                       sdkVersion: "3.3.0",
                                       locale: { language })
        _ = fetch(source)
        XCTAssertEqual(transport.lastBody?["locale"] as? String, "en")
        // The host switched language after the source was built — the next sync must ask
        // for the new one, not the one captured at construction.
        language = "ar"
        _ = fetch(source)
        XCTAssertEqual(transport.lastBody?["locale"] as? String, "ar")
    }
}
