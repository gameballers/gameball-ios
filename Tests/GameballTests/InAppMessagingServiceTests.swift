...
final class InAppMessagingServiceTests: XCTestCase {
    ...
    /// Floor retries the service booked, fired by hand. `(delay, work)`.
    ...
    /// An instant on a fixed day, named by the wall clock of the machine running the suite.
    /// Quiet hours are judged on the device's own clock, so a local literal is what keeps
    /// these tests machine-independent. The default `clock` may fall inside the nightly
    /// window on some machines — so every test below sets it.
    ///
    ...
    private func local(_ hour: Int, _ minute: Int = 0, dayOffset: Int = 0) -> Date {
    ...
    /// The replay path has its own quiet-hours gate. A message deferred at 21:59 and released
    /// by a dismissal at 22:01 would otherwise be the one thing quiet hours cannot stop. It is
    /// dropped, not held: a quiet window is hours long, and a message released at 08:00 would
    /// arrive as a non sequitur to whatever it was answering the night before.
    func testADeferredMessageIsDroppedOnceQuietHoursBegin() {
        clock = local(21, 30)
        let campaigns = [makeCampaign(campaignId: 1, priority: 9, responseIndex: 0),
                         makeCampaign(campaignId: 2, priority: 1,
                                      trigger: .event(name: "e", filters: []),
        ...
        // The cooldown is zero, so the display floor cannot be what stops it.
        XCTAssertEqual(presenter.presentedMessages.count, 1,
                       "the deferred message slipped through inside quiet hours")
        XCTAssertTrue(service.deferredMessages.isEmpty, "dropped, not held: the window is hours long")

        // The next morning brings nothing back.
        clock = local(8, 0, dayOffset: 1)
        service.onDisplayOpportunity()
        drain(service)
        XCTAssertEqual(presenter.presentedMessages.count, 1)
    }
    ...
    // MARK: - Same-trigger runner-up

    func testTheRunnerUpOnTheSameTriggerWaitsBehindTheWinner() {
        clock = local(12, 0)
        let campaigns = [makeCampaign(campaignId: 1, priority: 100,
                                      trigger: .event(name: "add_to_cart", filters: []),
                                      responseIndex: 0),
                         makeCampaign(campaignId: 2, priority: 50,
                                      trigger: .event(name: "add_to_cart", filters: []),
                                      responseIndex: 1)]
        let (service, _) = startedService(campaigns)
        service.onCustomEvent(name: "add_to_cart", properties: [:])
        drain(service)
        XCTAssertEqual(presenter.presentedMessages.map { $0.id }, ["1"])
        XCTAssertEqual(service.deferredMessages.map { $0.campaignId }, [2],
                       "the Normal campaign that fired on the same event is kept, not lost")
    ...
    func testARunnerUpParkedByTheFloorIsRetriedWhenItLapses() {
        clock = local(12, 0)
        let campaigns = [makeCampaign(campaignId: 1, priority: 100,
                                      trigger: .event(name: "add_to_cart", filters: []),
                                      responseIndex: 0),
                         makeCampaign(campaignId: 2, priority: 50,
                                      trigger: .event(name: "add_to_cart", filters: []),
                                      responseIndex: 1)]
        let (service, _) = startedService(campaigns, cooldown: 10)
        service.onCustomEvent(name: "add_to_cart", properties: [:])
        drain(service)
        presenter.reportShown() // the floor starts now
        drain(service)
        XCTAssertEqual(service.deferredMessages.map { $0.campaignId }, [2])

        // Dismissed 3 s in: 7 s of floor remain, so the runner-up is held and a retry booked.
        clock = clock.addingTimeInterval(3)
        presenter.reportDismissed()
        drain(service)
        XCTAssertEqual(presenter.presentedMessages.count, 1)
        XCTAssertEqual(service.deferredMessages.map { $0.campaignId }, [2])
        XCTAssertEqual(scheduled.count, 1, "one retry, timed to the end of the floor")
        XCTAssertEqual(scheduled[0].0, 7, accuracy: 0.001)
    ...
    func testAnOrientationMismatchDefersTheMessage() {
        presenter.obstacle = .orientationMismatch
        let (service, _) = startedService([makeCampaign(campaignId: 7)])
        XCTAssertEqual(service.deferredMessages.map { $0.campaignId }, [7])
        XCTAssertTrue(presenter.presentedMessages.isEmpty)
        // The rotation arrives as a display opportunity.
        presenter.obstacle = nil
        service.onDisplayOpportunity()
        drain(service)
        XCTAssertEqual(presenter.presentedMessages.map { $0.id }, ["7"])
