...
final class ArtworkPrefetcherTests: XCTestCase {
    ...
    /// Cleartext artwork is refused before any request leaves the device, and the campaign
    /// is then passed over exactly like one whose artwork failed to load — the same outcome
    /// App Transport Security produces, but stated by the SDK and independent of how the host
    /// configured ATS.
    func testCleartextArtworkIsRefusedWithoutAFetch() {
        IAMImageStub.set(.init(data: png), for: "http://example.com/plain.png")

        let prefetcher = makePrefetcher()
        let plain = campaign(1, image: "http://example.com/plain.png")
        warm(prefetcher, [plain])

        XCTAssertFalse(prefetcher.isReady(plain), "http artwork never counts as ready")
        XCTAssertTrue(IAMImageStub.requestedURLs.isEmpty, "refused before any fetch")
    }

    func testCleartextIconRefusesTheWholeCampaign() {
        IAMImageStub.set(.init(data: png), for: "https://example.com/ok.png")
        IAMImageStub.set(.init(data: png), for: "http://example.com/icon.png")

        let prefetcher = makePrefetcher()
        let mixed = campaign(2, image: "https://example.com/ok.png", icon: "http://example.com/icon.png")
        warm(prefetcher, [mixed])

        XCTAssertFalse(prefetcher.isReady(mixed))
        XCTAssertEqual(IAMImageStub.requestedURLs, ["https://example.com/ok.png"],
                       "only the https half was attempted")
    }
}
