...
final class MessageWindowPresenterTests: XCTestCase {
    ...
    private func fullscreen(_ orientation: GameballMessageOrientation) -> PresentationContext {
        return PresentationContext(message: makeMessage(type: .fullscreen, orientation: orientation))
    }

    func testPortraitOnlyMessageWaitsWhileTheDeviceIsLandscape() {
        let presenter = makePresenter()
        presenter.orientationProvider = { .landscapeLeft }
        XCTAssertEqual(presenter.present(context: fullscreen(.portrait), handlers: handlers()),
                       .orientationMismatch)
        XCTAssertFalse(presenter.isShowing)
        XCTAssertTrue(builtViews.isEmpty, "nothing is built for a message that is waiting")
    }

    func testLandscapeOnlyMessageWaitsWhileTheDeviceIsPortrait() {
        let presenter = makePresenter()
        presenter.orientationProvider = { .portrait }
        XCTAssertEqual(presenter.present(context: fullscreen(.landscape), handlers: handlers()),
                       .orientationMismatch)
    }

    func testMatchingOrientationPresents() {
        let presenter = makePresenter()
        presenter.orientationProvider = { .landscapeRight }
        XCTAssertNil(presenter.present(context: fullscreen(.landscape), handlers: handlers()))
        XCTAssertTrue(presenter.isShowing)
    }

    func testAnyOrientationPresentsHoweverTheDeviceIsHeld() {
        for orientation in [UIInterfaceOrientation.portrait, .landscapeLeft, .portraitUpsideDown] {
            let presenter = makePresenter()
            presenter.orientationProvider = { orientation }
            XCTAssertNil(presenter.present(context: fullscreen(.any), handlers: handlers()),
                         "\(orientation.rawValue)")
        }
    }

    func testUnknownOrientationDoesNotBlockAMessage() {
        let presenter = makePresenter()
        presenter.orientationProvider = { nil }
        XCTAssertNil(presenter.present(context: fullscreen(.portrait), handlers: handlers()))
    }
