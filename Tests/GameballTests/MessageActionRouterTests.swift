...
final class MessageActionRouterTests: XCTestCase {
    ...
    func testRequestPushPermissionAsksThenDismisses() {
        var asked = 0
        var dismissed = 0
        let router = MessageActionRouter(openURL: { _, _ in },
                                         navigate: { _, _ in },
                                         dismiss: { dismissed += 1 },
                                         requestPushPermission: { asked += 1 })
        XCTAssertNil(router.perform(.requestPushPermission))
