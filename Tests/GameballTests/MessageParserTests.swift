...
final class MessageParserTests: XCTestCase {
    ...
    func testFilterWithBadOperatorIsDroppedIndividually() {
        let parsed = parseOne(["trigger": [
            ...
            "name": "place_order",
            "metadataFilters": [
                ["name": "price", "operator": "Regex", "value": 100],
    ...
    func testRequestPushPermissionParsesToItsOwnAction() {
        let parsed = parseOne(content: ["action": ["type": "request_push_permission"]])
        guard case .some(.requestPushPermission) = parsed?.message.clickAction else {
            return XCTFail("request_push_permission must not degrade to unsupported or dismiss")
        }
    }

    func testUnimplementedActionTypesParseAsUnsupported() {
        for type in ["log_event", "log_attribute"] {
    ...
    /// A missing `closeBehaviour` defaults to "both", which would report `showCloseButton`
    /// as `true` for every slideup. A slideup never renders one — dismissal is the swipe and
    /// the timer — so the flag is reported honestly rather than describing something the view
    /// does not draw.
    ...
    /// `quietHours` is parsed rather than ignored — see QuietHoursTests — so the keys used
    /// here as examples are ones the parser genuinely does not read.
