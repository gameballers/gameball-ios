...
///
/// Nothing records *which* URLs failed, deliberately. The only use for such a record would be
/// skipping a URL that failed before, and that is the opposite of what this wants — a failure is
/// more often the network than the asset, so the next warm should try again. `load` consults the
/// image cache
...
final class ArtworkPrefetcher {
    ...
    private func load(_ url: URL, completion: @escaping () -> Void) {
        ...
        // Cleartext artwork is refused, not attempted. App Transport Security blocks it by
        // default anyway, but a host that has opened ATS up would otherwise display a
        // campaign the other SDKs never show. One rule on every platform: https only, and
        // the campaign is passed over exactly as it is when artwork fails to load.
        if url.scheme?.lowercased() == "http" {
            iamLog("artwork \(url) is served over http and is refused — in-app artwork must be https")
            completion()
            return
        }
