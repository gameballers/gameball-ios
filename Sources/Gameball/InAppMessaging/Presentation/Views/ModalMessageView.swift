...
final class ModalMessageView: UIView, InAppMessageView {
    ...
    func install(in container: UIView) {
        ...
        // more than `copyReserve` allows for — the reserve binds first there — and the card
        // absorbing that is better than the artwork losing a hundred points to it. The
    ...
    func present(completion: (() -> Void)?) {
        ...
        // Fade only, with no scale: a fade is the one motion property all four SDKs
        // reproduce identically — a curve or a spring is not.
        alpha = 0
