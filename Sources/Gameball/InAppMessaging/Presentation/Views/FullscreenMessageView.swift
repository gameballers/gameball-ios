...
final class FullscreenMessageView: UIView, InAppMessageView {
    ...
    private func build() {
        ...
        if !message.buttons.isEmpty {
            // A plain stack, not `MessageButtonStack`: the composer preview stacks fullscreen
            // buttons full-width in a column, and so does the Flutter SDK. The adaptive row
            // is a modal affordance.
