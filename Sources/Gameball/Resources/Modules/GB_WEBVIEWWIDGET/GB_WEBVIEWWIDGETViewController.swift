...
class GB_WEBVIEWWIDGETViewController: BaseViewController {
    ...
    var widgetEventCallback: ((String) -> Void)?
...
extension GB_WEBVIEWWIDGETViewController: WKScriptMessageHandler {
    ...
    /// The body is the raw event string, forwarded verbatim to `widgetEventCallback`.
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        guard message.name == "widgetEvent", let raw = message.body as? String else { return }
        widgetEventCallback?(raw)
    }
}
